...
public class RedisLock {
    ...
    private static final int DEFAULT_LOCK_EXPIRE_MILL_SECONDS = 1000 * 30;
    ...
    public RedisLock(String lockKey, int lockExpireTimeout, int lockWaitTimeOut) {
        this.lockKey = lockKey;
        this.lockExpireTimeout = lockExpireTimeout;
    ...
    public RedisLock newInstance(String lockKey, int lockWaitTimeOut) {
        ...
        RedisLock lock = new RedisLock(lockKey, DEFAULT_LOCK_EXPIRE_MILL_SECONDS, lockWaitTimeOut);
    ...
    public boolean lock() {
        ...
        String redis_value = String.valueOf(System.currentTimeMillis() + this.lockExpireTimeout);
        while (lockWaitMillSeconds > 0) {
            ...
            if (lock) {
                ...
                if (!StringUtils.isEmpty(oldRedisValue) && Long.parseLong(oldRedisValue) < currentTimeMillSeconds) {
                    String currentRedisValue = this.getAndSet(String.valueOf(currentTimeMillSeconds + this.lockExpireTimeout));
    ...
    public int getLockExpireTimeout() {
        return lockExpireTimeout;
    }

    public void setLockExpireTimeout(int lockExpireTimeout) {
        this.lockExpireTimeout = lockExpireTimeout;
    }
